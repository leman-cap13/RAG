...
def get_cached_answer(question, top_k):
    hits = cahce.check(prompt=question, filter_expression=Num('top_k') == top_k, num_results=1)
    return json.loads(hits[0]['response']) if hits else None


def set_cached_answer(question, top_k, payload):
    cahce.store(prompt=question, response=json.dumps(payload), filters={'top_k': top_k})
